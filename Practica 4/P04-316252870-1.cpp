...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		color = glm::vec3(1.0f, 1.0f, 1.0f);   // Color cabina 
		...
		modelaux = model;
		modelCabina = modelaux;
		model = glm::scale(model, glm::vec3(5.0f, 3.33f, 2.5f));
		...
		model = modelaux;	// Para descartar la escala
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion1()), glm::vec3(0.0f, 0.0f, 1.0f)); //Sin geometria/esfera
		...
		model = modelaux;  //Descartamos escala, heredamos traslacion 
		...
		//model = glm::mat4(1.0);   Se quita porque se quiere heredar del brazo 1
		...
		model = modelaux; //Descartamos escala, heredamos la rotacion y traslacion
		...
		//Ejercicio de clase: completar: brazo, articulacion 4 y canasta
		...
		// Practica: 1.- Terminar la grua

		model = modelaux;  //Descartamos la escala
		modelaux = modelCabina;   //Regresamos a la cabina
		model = modelaux;   //Regresamos la matriz principal a la cabina
		...
		// Llantas
		model = modelaux;  //Regreso a la cabina

		// Articulacion y llanta 1 con geometria 
		model = glm::translate(model, glm::vec3(-4.0f, -3.0f, 3.0f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion5()), glm::vec3(0.0f, 0.0f, 1.0f));

		modelaux = model;    //Descartamos la escala y heredar
		...
		// Llanta 
		model = modelaux;    // es necesario heredar la rotacion y la traslacion cuando se usa una geometria
		...
		// Llanta 2 con geometria
		modelaux = modelCabina;
		model = modelaux;      
		model = glm::translate(model, glm::vec3(-4.0f, -3.0f, -3.0f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion6()), glm::vec3(0.0f, 0.0f, 1.0f));

		modelaux = model;    //Descartamos la escala
		model = glm::scale(model, glm::vec3(1.0f, 1.0f, 1.0f));
